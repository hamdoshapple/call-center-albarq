...
export function Sidebar({ open, onClose }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          'fixed inset-y-0 z-40 flex w-64 flex-col bg-sidebar text-sidebar-foreground transition-transform duration-200 lg:static lg:translate-x-0',
          'start-0',
      ...
      >
        <div className="flex h-16 items-center justify-between gap-2 border-b border-sidebar-border px-4">
          ...
          <button onClick={onClose} className="rounded-md p-1.5 hover:bg-white/10 lg:hidden">
            <X className="h-5 w-5" />
          </button>
        </div>
                  ...
                  {items.map((item) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="truncate">{t(`nav.${item.module}`)}</span>
                      </NavLink>
                    );
                  })}
                </div>
