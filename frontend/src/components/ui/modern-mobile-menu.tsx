...
const InteractiveMenu: React.FC<InteractiveMenuProps> = ({ items, accentColor, activeKey, onChange }) => {
  ...
  useEffect(() => {
    if (!activeKey) return;
    const nextIndex = finalItems.findIndex((item) => item.key === activeKey);
    if (nextIndex >= 0 && nextIndex !== activeIndex) {
      setActiveIndex(nextIndex);
    }
  }, [activeKey, finalItems, activeIndex]);
  ...
  const handleItemClick = (index: number) => {
    ...
    onChange?.(item?.key || String(index), index);
  };
