...
export function SubscriberPortalPage() {
  ...
  async function checkPushPermission() {
    ...
    if (step === 'home' && isPwaMode && config.enableNotifications !== false) {
      setPushPromptOpen(permission !== 'granted');
    }
  }
  ...
  useEffect(() => {
    ...
    const onFocus = () => checkPushPermission();
    const onVisibility = () => {
      if (!document.hidden) checkPushPermission();
    };
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [step, isPwaMode, token, config.enableNotifications]);
  ...
  return (
    <div dir="rtl" className="min-h-screen bg-slate-100 pb-32 pt-[92px] text-slate-950">
        ...
        <div className="fixed inset-0 z-[120] flex items-center justify-center bg-slate-950/60 p-5 backdrop-blur-sm">
          <div className="w-full max-w-sm overflow-hidden rounded-[32px] bg-white shadow-2xl">
            ...
            >
              ...
              <div className="absolute -bottom-12 right-12 h-36 w-36 rounded-full bg-white/10" />

              <div className="relative flex items-start gap-4">
                <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-3xl bg-white/20 backdrop-blur">
                  <Bell className="h-7 w-7" />
                </div>

                <div>
                  <h3 className="text-2xl font-black">فعّل الإشعارات</h3>
                  <p className="mt-2 text-sm leading-7 text-white/85">
                    حتى توصلك تنبيهات التجديد، انتهاء الاشتراك، الديون، وتحديثات التذاكر أول بأول.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-3 p-5">
              <div className="rounded-3xl bg-slate-50 p-4">
                <div className="mb-3 text-sm font-black text-slate-700">راح تستفاد من:</div>

                <div className="space-y-3 text-sm font-bold text-slate-600">
                  <div className="flex items-center gap-3">
                    <span className="flex h-8 w-8 items-center justify-center rounded-2xl bg-red-50 text-red-600">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    إشعارات الديون والمدفوعات
                  </div>

                  <div className="flex items-center gap-3">
                    <span className="flex h-8 w-8 items-center justify-center rounded-2xl bg-blue-50 text-blue-600">
                      <MessageCircle className="h-4 w-4" />
                    </span>
                    متابعة تذاكر الدعم الفني
                  </div>
                </div>
              </div>

              {pushPermission === 'denied' && (
                <div className="rounded-2xl bg-red-50 p-3 text-sm font-bold leading-6 text-red-600">
                  الإشعارات محظورة من إعدادات الجهاز أو المتصفح. افتح إعدادات التطبيق وفعّل السماح بالإشعارات.
                </div>
              )}

              {pushStatus && (
                <div className="rounded-2xl bg-slate-50 p-3 text-center text-sm font-bold text-slate-600">
                  {pushStatus}
                </div>
              )}
              ...
              <button
                ...
                disabled={pushChecking || pushPermission === 'denied'}
                className="h-14 w-full rounded-2xl text-lg font-black text-white disabled:opacity-50"
                style={{ backgroundColor: primary }}
              >
                {pushChecking ? 'جاري التفعيل...' : 'تفعيل الإشعارات الآن'}
              </button>
      ...
      <div className="mx-auto max-w-md px-5">
        <header className="fixed inset-x-0 top-0 z-50 mx-auto flex max-w-md items-center justify-between bg-slate-100/95 px-5 pb-3 pt-[calc(env(safe-area-inset-top)+14px)] backdrop-blur">
          <div className="flex items-center gap-3">
            {config.logoUrl && (
              <img
                src={assetUrl(config.logoUrl)}
