...
export async function enableEmployeePush() {
  ...
  if (permission !== 'granted') {
    return { ok: false, message: 'لم يتم السماح بالإشعارات' };
  }

  const reg = await navigator.serviceWorker.register('/employee-sw.js', { scope: '/employee' });
  ...
  if (!saveRes.ok) {
    return { ok: false, message: saveData.message || saveData.error || 'تعذر حفظ الإشعارات' };
  }
